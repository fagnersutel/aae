...
col_vals=c('Score 0-200'='yellow', 'Score 200-400'='orange',
           'Score 400-600'='green','Score 600-800'='darkgreen',
           'Score 800-1000'='#0080FE')
ggmap(mh_map_set_dois) +
...
leaflet(dados) %>%
    addTiles(group="OSM") %>%
    addHeatmap(group="heat", lng=dados$V1, lat=dados$V2, max=30, blur = 60) %>% 
    addCircles(~V1, ~V2, popup=dados$newrow,  weight = 0.1, radius=1, color="red", stroke = TRUE, fillOpacity = 0.8) %>% 
    addLegend("bottomright", colors= "#ffa500", labels="Porto alegre", title="Concentração de Alvaras")
    #%>%
...
leaflet(dados) %>%
    ...
    addLegend("bottomright", colors= "#ffa500", labels="Comércio e Serviços", title="Alvaras de:")
